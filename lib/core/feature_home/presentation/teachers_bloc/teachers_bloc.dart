...
class TeachersViewBloc extends Bloc<TeachersViewEvent, TeachersViewState> {
  TeachersViewBloc() : super(TeachersInitial()) {
    on<LoadingTeachersViewEvent>((event, emit) async {
      ...
      final role = configClass?.userLogin?.user?.role;

      print("Teachers -> role = $role");
      print("Teachers -> academyId = ${event.academyId}");
      ...
      if (!hasInternet) {
        ...
        if (role == "MANAGER") {
          ...
          final cachedTeachers =
              await SharedPreferencesHelper.getManagerTeachers();

          if (cachedTeachers != null &&
              cachedTeachers.doc != null &&
              cachedTeachers.doc!.isNotEmpty) {
            print("Manager Teachers Cache Found");

            emit(TeachersSuccessState(teachersModel: cachedTeachers));
          } else {
            print("Manager Teachers Cache Empty");
          ...
          final cachedTeachers =
              await SharedPreferencesHelper.getTeachersForAcademy(
                event.academyId,
              );
          ...
          if (cachedTeachers != null &&
              cachedTeachers.doc != null &&
              cachedTeachers.doc!.isNotEmpty) {
            print("Teachers Cache Found for academy ${event.academyId}");

            emit(TeachersSuccessState(teachersModel: cachedTeachers));
          } else {
            print("Teachers Cache Empty for academy ${event.academyId}");
      ...
      try {
        ...
        final TeachersModel teachersModel = await TeachersApi.getAllTeachers(
          event.context,
          event.academyId,
        );
        ...
        print(
          "Teachers -> statusCode = "
          "${teachersModel.statusCode}",
        );

        print(
          "Teachers -> message = "
          "${teachersModel.message}",
        );
        ...
        if (teachersModel.statusCode == 200) {
          ...
          print("========== TEACHERS CACHE TEST ==========");
          print("academyId = ${event.academyId}");
          print("role = $role");
          print("teachers count = ${teachersModel.doc?.length}");
          // ----------------------------------------
          // Manager
          // ----------------------------------------

          if (role == "MANAGER") {
            await SharedPreferencesHelper.saveManagerTeachers(teachersModel);

            print("Manager Teachers saved to cache");
          }
          // ----------------------------------------
          // User
          // ----------------------------------------
          else {
            await SharedPreferencesHelper.saveTeachersForAcademy(
              event.academyId,
              teachersModel,
            );

            print(
              "Teachers saved for academy "
              "${event.academyId}",
            );
          }
          ...
          emit(TeachersSuccessState(teachersModel: teachersModel));
        ...
        if (teachersModel.statusCode == 401) {
          ...
          showMessage(event.context, "يجب تسجيل الدخول بحساب مستخدم", true);

          emit(TeachersUnauthorizedState(message: teachersModel.message));
        ...
        if (teachersModel.statusCode == 403) {
          ...
          showMessage(event.context, "هذه العملية متاحة للمدير فقط", true);

          emit(TeachersForbiddenState(message: teachersModel.message));
        ...
        if (teachersModel.statusCode == 500) {
          ...
          showMessage(event.context, "رقم المعهد غير صحيح", true);

          emit(TeachersServerErrorState(message: teachersModel.message));
        ...
        showMessage(event.context, "حدث خطأ ما، حاول مرة أخرى", true);

        emit(TeachersErrorState(message: teachersModel.message));
      }
      // ==========================================
      // 11. SocketException
      // ==========================================
      ...
      on TimeoutException {
        ...
        if (role == "MANAGER") {
          ...
          if (cachedTeachers != null &&
            ...
            emit(TeachersErrorState(message: "Timeout"));
          }
        } else {
          final cachedTeachers =
              await SharedPreferencesHelper.getTeachersForAcademy(
      ...
      // ==========================================
        ...
        if (role == "MANAGER") {
          ...
          if (cachedTeachers != null &&
            ...
            emit(TeachersErrorState(message: e.toString()));
          }
        } else {
          final cachedTeachers =
              await SharedPreferencesHelper.getTeachersForAcademy(
